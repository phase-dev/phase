...
class TCPClient(_Connection):
    ...
    def convert_to_ssl(self, cert=None, sni=None, method=TLSv1_METHOD, options=None, cipher_list=None, key=None):
        """
        ...
        if cert != None and key != None:
            try:
		if isinstance(key,crypto.PKeyType):
	                context.use_privatekey(key)
		else:
			raise SSL.Error
		if isinstance(cert,crypto.X509Type):
