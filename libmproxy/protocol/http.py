...
class HTTPRequest(HTTPMessage):
	...
	if self.form_out == "absolute":			
		request_header=self.method+" "+self.scheme+"://"+self.host+self.path+" "+self.http_version()+"\r\n"	
	elif self.form_out == "origin":
		request_header=self.method+" "+self.path+" "+self.http_version()+"\r\n"
	elif self.form_out == "asterisk-form":
		request_header=self.method+" * "+self.http_version()+"\r\n"
	return request_header+str(self.headers)
...
class HTTPResponse(HTTPMessage):
    ...
    def get_headers(self,remove_transfer_encoding=False):
	headers=self.headers
	if "Transfer-Encoding" in headers and remove_transfer_encoding==True:
...
class HTTPFlow(Flow):
	...
	state["response"]["content"]=b64encode(state["response"]["content"])
	state["request"]["content"]=b64encode(state["request"]["content"])
	return b64encode(zlib.compress(json.dumps(state)))
    ...
    def load(cls,state):
	f=cls(None,None)
	state=b64decode(state)
	state=zlib.decompress(state)	
	state=json.loads(state)
	state["response"]["content"]=b64decode(state["response"]["content"])
	state["request"]["content"]=b64decode(state["request"]["content"])	
	f._load_state(state)	
	return f
...
class HTTPHandler(ProtocolHandler, TemporaryServerChangeMixin):
	...
	if self.c.server_conn.ssl_attempted:
		self.c.establish_ssl(client=True)
        response = http_status.RESPONSES.get(code, "Unknown")
        html_content = error.error_page % (code, response, code, message)
    ...
    def ssl_upgrade(self):
        ...
        """
	self.c.server_conn.ssl_attempted=True
