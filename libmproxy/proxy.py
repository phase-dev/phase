...
class ProxyConfig:
    def __init__(self, confdir=CONF_DIR, clientcerts=None, 
                       ...
                       ciphers=None, certstore=None, disable_client_ssl=False
                ):
	...
	self.disable_client_ssl=disable_client_ssl
...
class ServerConnection(tcp.TCPClient, stateobject.SimpleStateObject):
	...
	self.ssl_attempted=True
	if clientcerts == None:
		public=None
		private=None
	else:
		public=clientcerts.public
		private=clientcerts.private
        try:
            self.convert_to_ssl(cert=public, key=private, sni=sni)
            self.timestamp_ssl_setup = utils.timestamp()
...
class ConnectionHandler:
    ...
    def _handle_ssl(self):
        ...
        if self.config.transparent_proxy:
            self.ssl_attempted = client_ssl = server_ssl = (self.server_conn.address.port in self.config.transparent_proxy["sslports"])
        elif self.config.reverse_proxy:
            server_ssl = (self.config.reverse_proxy[0] == "https")
            if not self.config.disable_client_ssl and (self.config.reverse_proxy[0] == "https"):
		client_ssl=True
		self.ssl_attempted=True
    ...
    def find_cert(self):
	#Modified to always create certificate using host
